...
(function (ts) {
    ...
    function createTypeChecker(program, fullTypeCheck) {
        ...
        function signatureHasCorrectArity(node, signature) {
            ...
            var isCorrect = args.length >= signature.minArgumentCount && (signature.hasRestParameter || args.length <= signature.parameters.length) && (!node.typeArguments || signature.typeParameters && node.typeArguments.length === signature.typeParameters.length);
            if (!isCorrect) {
                if (args.hasTrailingComma || ts.forEach(args, function (arg) { return arg.kind === 147 /* OmittedExpression */; })) {
                    return true;
                }
            }
            return isCorrect;
...
(function (ts) {
    ...
    function createCompilerHost(options, defaultLibUrl) {
        ...
        return {
            ...
            getDefaultLibFilename: function () {
                if (defaultLibUrl) {
                    return defaultLibUrl;
                }
                var filename = ts.combinePaths(ts.getDirectoryPath(ts.normalizePath(sys.getExecutingFilePath())), "./core/typescript/lib.d.ts");
                return filename;
            },
            writeFile: writeFile,
    ...
    function executeCommandLine(args, defaultLibUrl) {
        ...
        var defaultCompilerHost = createCompilerHost(compilerOptions, defaultLibUrl);
