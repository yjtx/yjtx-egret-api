...
function resultTip(options) {
    ...
    var pop = $('<div style="left:50%;top:30%;position: fixed;" class="loading"><span class="pop_showmsg"><span class="' + cls + '">' + options.msg + '</span></span></div>');

    pop.appendTo($('body')).fadeIn(function () {
        pop.css({
            marginLeft: -pop.innerWidth() / 2
        }); //水平居中
    }).delay(150000).fadeOut(function () {
        pop.remove();
        ...
        form.ajaxSubmit({
            ...
            success: function (data) {
                ...
                if(data.type)
                {
                    ...
                    else if(data.type==="modal-content-add")
                    {
                        $("#"+data.name).html(data.info);
                    }
                    if(data.modal!==null)
                    {
                        $(".modal").modal("hide");
                        $(".modal-backdrop").remove();
                        eval("$('#"+data.modal+"').modal('"+data.modalshow+"')");
                    }
                    return;
                }
                if (data.referer) {
    ...
    $.ajax({
        ...
        success: function (data, textStatus, jqXHR) {
            ...
            if (data.state === 'success') {
                ...
                console.log("error frontend J_Modal");
            }
        }			
    });
}
...
function comment_reply_bak(obj){
	$(".comments .comment-reply-submit").hide();
	var $this=$(obj);
	var $comment_body=$this.parents(".comments > .comment> .comment-body");
	var commentid=$this.parents(".comment").data("id");
...
function comment_submit(obj){
	...
	Wind.use('noty', function () {
                ...
                if($('#main-menu-user-nicename')!=null && $('#main-menu-user-nicename').text()=='')
                {
                    alert('未登录状态不能评论！');
                    return;
                }
